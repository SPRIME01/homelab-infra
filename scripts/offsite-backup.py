...
SOURCE_BACKUP_DIRS = os.getenv("SOURCE_BACKUP_DIRS", "/backups/postgresql,/backups/redis,/backups/influxdb,/backups/files,/backups/config-backup").split(',')
# File pattern to select within source dirs (e.g., latest .gpg files)
SOURCE_FILE_PATTERN = os.getenv("SOURCE_FILE_PATTERN", "*_????????_??????.*.gpg") # Adjust if primary backups aren't encrypted
...
S3_ENDPOINT_URL = os.getenv("S3_ENDPOINT_URL") # e.g., 'https://s3.us-east-1.amazonaws.com' or Backblaze/MinIO endpoint
S3_ACCESS_KEY = os.getenv("S3_ACCESS_KEY")
S3_SECRET_KEY = os.getenv("S3_SECRET_KEY")
S3_REGION = os.getenv("S3_REGION") # e.g., 'us-east-1', required for AWS
S3_BUCKET = os.getenv("S3_BUCKET")
S3_PREFIX = os.getenv("S3_PREFIX", "homelab-backups/") # Optional prefix within the bucket
...
GPG_RECIPIENT_OFFSITE = os.getenv("GPG_RECIPIENT_OFFSITE") # GPG Key ID for offsite encryption
...
def encrypt_file_gpg(filepath, recipient, staging_dir):
    ...
    encrypted_filename = f"{base_filename}.gpg" if not base_filename.endswith(".gpg") else base_filename
    ...
    if os.path.exists(encrypted_filepath):
        logging.warning(f"Encrypted file already exists in staging: {encrypted_filepath}. Skipping encryption.")
    ...
    try:
        run_command([
            "gpg",
            "--encrypt", "--yes", "--trust-model", "always", # Assume key is trusted in automated env
            "--recipient", recipient,
...
def get_s3_client():
    ...
    try:
        ...
        logging.info(f"Successfully initialized S3 client for endpoint {S3_ENDPOINT_URL}")
        return s3_client
...
def upload_and_verify(s3_client, local_filepath, bucket, object_key):
    ...
    try:
        ...
        if s3_etag == local_md5:
            logging.info(f"Verification successful: S3 ETag matches local MD5 for {object_key}")
            return True
        else:
            # Could be due to multipart upload, encryption settings on bucket, or provider differences
            logging.warning(f"Verification mismatch or inconclusive: S3 ETag '{s3_etag}' != Local MD5 '{local_md5}' for {object_key}.")
            logging.warning("This might be expected with multipart uploads or non-AWS providers. Manual check recommended if unsure.")
...
def apply_offsite_retention(s3_client, bucket, prefix):
    ...
    logging.info(f"Applying offsite retention policy (>{OFFSITE_RETENTION_DAYS} days) to s3://{bucket}/{prefix}")
    ...
    try:
            ...
            if 'Contents' in page:
                for obj in page['Contents']:
                    ...
                    if last_modified < cutoff_date:
                        logging.info(f"Marking for deletion (older than {cutoff_date}): {key} (LastModified: {last_modified})")
        ...
        if objects_to_delete:
            ...
            for i in range(0, len(objects_to_delete), 1000):
                ...
                logging.info(f"Deleted {deleted_count} objects. Encountered {error_count} errors in this chunk.")
                if error_count > 0:
...
def main():
    ...
    if ENCRYPT_OFFSITE and not GPG_RECIPIENT_OFFSITE:
        logging.error("Offsite encryption enabled, but GPG_RECIPIENT_OFFSITE is not set. Exiting.")
        sys.exit(1)
    ...
    with ensure_clean_dir(STAGING_DIR) as staging_dir:
        ...
        for source_dir in SOURCE_BACKUP_DIRS:
            ...
            for f_path in found_files:
                ...
                try:
                    ...
                    logging.warning(f"Could not determine prefix for file {filename}. Skipping.")
            ...
            for prefix, file_list in files_by_prefix.items():
                ...
                logging.info(f"Selected latest {len(selected_files)} file(s) for prefix '{prefix}' in {source_dir}: {selected_files}")
        ...
        for local_path in all_files_to_upload:
            ...
            # Encrypt if needed
            if ENCRYPT_OFFSITE:
                encrypted_path = encrypt_file_gpg(local_path, GPG_RECIPIENT_OFFSITE, staging_dir)
                if not encrypted_path:
                    logging.error(f"Skipping upload for {local_path} due to encryption failure.")
                ...
                cleanup_staging_file = encrypted_path # Mark for cleanup after upload
                 ...
                 logging.warning(f"File {local_path} appears unencrypted, and ENCRYPT_OFFSITE is false. Uploading as is.")

            # Define S3 object key
            relative_path = os.path.relpath(local_path, start=os.path.dirname(os.path.dirname(local_path))) # Get path relative to parent of source_dir
            object_key = os.path.join(S3_PREFIX, relative_path).replace("\\", "/") # Ensure forward slashes
            ...
            if cleanup_staging_file and os.path.exists(cleanup_staging_file):
                try:
                    ...
                    logging.warning(f"Failed to clean up staged file {cleanup_staging_file}: {e}")
    ...
    logging.info(f"Uploaded successfully: {upload_success_count}, Failed: {upload_failure_count}")
...
if __name__ == "__main__":
    ...
    if ENCRYPT_OFFSITE:
        if shutil.which("gpg") is None:
            logging.error("Missing required command-line tool: gpg (required because ENCRYPT_OFFSITE=true)")
            sys.exit(2)
        if not GPG_RECIPIENT_OFFSITE:
             logging.error("ENCRYPT_OFFSITE=true but GPG_RECIPIENT_OFFSITE is not set.")
             sys.exit(3)
        logging.info(f"Offsite encryption enabled for recipient: {GPG_RECIPIENT_OFFSITE}")
        logging.warning("Ensure the GPG public key is imported and trusted where this script runs.")

    # --- Cost Management Note ---
    logging.info("Note: Cloud storage costs depend on provider, storage class, amount stored, and data transfer.")
    logging.info("Monitor costs via your cloud provider's console. Consider using S3 Lifecycle Policies for transitions (e.g., to Glacier) or expiration.")
