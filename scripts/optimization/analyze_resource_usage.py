...
# --- Configuration ---
PROMETHEUS_URL = os.getenv("PROMETHEUS_URL", "http://prometheus.homelab:9090") # URL of your Prometheus server
...
NODE_DISK_HIGH_THRESHOLD = float(os.getenv("NODE_DISK_HIGH_THRESHOLD", "80")) # % usage (for root filesystem)
POD_CPU_LIMIT_NEAR_THRESHOLD = float(os.getenv("POD_CPU_LIMIT_NEAR_THRESHOLD", "90")) # % of limit reached
POD_MEM_LIMIT_NEAR_THRESHOLD = float(os.getenv("POD_MEM_LIMIT_NEAR_THRESHOLD", "90")) # % of limit reached
POD_CPU_REQUEST_LOW_THRESHOLD = float(os.getenv("POD_CPU_REQUEST_LOW_THRESHOLD", "20")) # % of request used (potential over-request)
POD_MEM_REQUEST_LOW_THRESHOLD = float(os.getenv("POD_MEM_REQUEST_LOW_THRESHOLD", "30")) # % of request used (potential over-request)

QUERY_DURATION = os.getenv("QUERY_DURATION", "1h") # Time window for average usage queries
...
def analyze_node_resources():
    ...
    if cpu_results:
        for item in cpu_results:
            node = item['metric'].get('instance', 'unknown').split(':')[0] # Extract node name
            usage = float(item['value'][1])
            logger.info(f"Node '{node}': Avg CPU Usage ({QUERY_DURATION}) = {usage:.2f}%")
    ...
    mem_query = f'(1 - (node_memory_MemAvailable_bytes / node_memory_MemTotal_bytes)) * 100'
...
def analyze_pod_resources():
    ...
    if cpu_limit_results:
        for item in cpu_limit_results:
            ...
            logger.info(f"Pod '{ns}/{pod}': Avg CPU Usage vs Limit ({QUERY_DURATION}) = {usage_percent_limit:.2f}%")
    ...
    if mem_limit_results:
        for item in mem_limit_results:
            ...
            pod = item['metric'].get('pod', 'unknown')
            usage_percent_limit = float(item['value'][1])
            logger.info(f"Pod '{ns}/{pod}': Current Memory Usage vs Limit = {usage_percent_limit:.2f}%")
...
def main():
    ...
    logger.info("Prerequisites: Ensure node-exporter, kube-state-metrics, and metrics-server are running and scraped by Prometheus.")
    ...
    if not all_recommendations:
        logger.info("No major resource optimization recommendations found based on current thresholds.")
    else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.critical("Missing required Python library: requests. Please install it (`pip install requests`).")
        sys.exit(2)
