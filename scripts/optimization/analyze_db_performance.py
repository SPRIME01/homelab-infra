...
SLOW_QUERY_THRESHOLD_MS = int(
    os.getenv("SLOW_QUERY_THRESHOLD_MS", "100")
)  # Queries longer than this are flagged
TOP_N_SLOW_QUERIES = int(os.getenv("TOP_N_SLOW_QUERIES", "10"))
MIN_SEQ_SCANS_FOR_INDEX_REC = int(
    os.getenv("MIN_SEQ_SCANS_FOR_INDEX_REC", "100")
)  # Min seq scans to suggest index
...
try:
    ...
    logger.critical(
        "Missing required Python library: psycopg2. Please install it (`pip install psycopg2-binary`)."
    )
    sys.exit(2)
...
def get_db_connection():
    ...
    try:
        ...
        logger.info(
            f"Successfully connected to PostgreSQL database '{PG_DATABASE}' on {PG_HOST}."
        )
        return conn
...
def check_pg_stat_statements(conn):
    ...
    if result:
        ...
        logger.warning(
            "Enable it for detailed query performance analysis: CREATE EXTENSION IF NOT EXISTS pg_stat_statements;"
        )
        return False
...
def analyze_slow_queries(conn):
    ...
    try:
        results = execute_query(
            conn, query, (PG_DATABASE, SLOW_QUERY_THRESHOLD_MS, TOP_N_SLOW_QUERIES)
        )
        if not results:
        ...
        logger.warning(
            f"Found {len(results)} slow queries (avg > {SLOW_QUERY_THRESHOLD_MS}ms):"
        )
        for row in results:
            rec = (
                f"Slow Query Found:\n"
...
def analyze_index_usage(conn):
    ...
    try:
        ...
        if not results:
            logger.info(
                "No tables found with high sequential scan counts based on current heuristics."
            )
            return recommendations
        ...
        for row in results:
            rec = (
                f"High Sequential Scans on Table '{row['table_name']}':\n"
...
def main():
    ...
    try:
        ...
        logger.info(
            "Placeholder: Add checks for unused indexes, vacuum status, connection stats..."
        )
    ...
    if not all_recommendations:
        logger.info(
            "No major database optimization recommendations found based on current checks."
        )
    else:
