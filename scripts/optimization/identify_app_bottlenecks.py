...
def run_kubectl(command, context=KUBECTL_CONTEXT, parse_json=False, check=True, timeout=60):
...
def parse_quantity(quantity_str):
    ...
    if quantity_str.endswith('m'): return float(quantity_str[:-1]) # CPU millicores -> treat as numeric value
    if quantity_str.endswith('ki'): return float(quantity_str[:-2]) * 1024
    if quantity_str.endswith('mi'): return float(quantity_str[:-2]) * 1024**2
    if quantity_str.endswith('gi'): return float(quantity_str[:-2]) * 1024**3
    # Add other units (k, M, G) if needed, being careful about base-10 vs base-2
    try: return float(quantity_str) # Assume cores or bytes
    ...
    for line in lines:
        ...
        if len(parts) >= 4:
            ...
            pod_usage.append({
                ...
                "cpu_val": parse_quantity(cpu_usage_str + "m"), # Treat CPU value as millicores for sorting
...
def main():
    ...
    logger.warning("NOTE: This script identifies high resource consumers as a STARTING POINT.")
    logger.warning("True bottleneck analysis requires application-specific profiling, tracing, and log analysis.")
    ...
    if top_cpu_pods:
        ...
        print("  Recommendation: Investigate these pods further. Check logs, consider profiling (e.g., pprof for Go, async-profiler for Java), or analyze application-specific metrics if available.")
    else:
    ...
    if top_mem_pods:
        ...
        print("  Recommendation: Investigate these pods for memory leaks or inefficient memory usage. Use memory profilers, analyze heap dumps, or check application metrics.")
    else:
