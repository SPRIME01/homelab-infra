...
NODE_CPU_WARN_THRESHOLD = float(
    os.getenv("NODE_CPU_WARN_THRESHOLD", "75")
)  # % usage avg 5m
NODE_CPU_FAIL_THRESHOLD = float(os.getenv("NODE_CPU_FAIL_THRESHOLD", "90"))
NODE_MEM_WARN_THRESHOLD = float(
    os.getenv("NODE_MEM_WARN_THRESHOLD", "80")
)  # % usage current
NODE_MEM_FAIL_THRESHOLD = float(os.getenv("NODE_MEM_FAIL_THRESHOLD", "95"))
NODE_DISK_WARN_THRESHOLD = float(
    os.getenv("NODE_DISK_WARN_THRESHOLD", "70")
)  # % usage current (root fs)
...
LOCAL_BACKUP_DIRS = os.getenv(
    "LOCAL_BACKUP_DIRS", "/backups/postgresql,/backups/files"
).split(",")
...
MAX_BACKUP_AGE_HOURS = int(
    os.getenv("MAX_BACKUP_AGE_HOURS", "26")
)  # Expect backups at least daily
...
)  # Prefix where backups are stored
    ...
    try:
        ...
        if log_output:
            ...
            if stderr:
                logger.debug(
                    f"Command stderr:\n{stderr}"
                )  # Debug level for stderr on success
...
def add_result(check_name, status, message, details=None, recommendation=None):
    ...
    if status == "WARN":
        logger.warning(f"Check '{check_name}': {status} - {message}")
    if status == "FAIL":
        logger.error(f"Check '{check_name}': {status} - {message}")
...
def check_system_resources():
    ...
    if not PROMETHEUS_URL:
        add_result(
            ...
            recommendation="Set PROMETHEUS_URL environment variable.",
        )
        return
    ...
    if cpu_results is None:
        add_result(
            f"{check_name}.CPU", "FAIL", "Failed to query Prometheus for CPU usage."
        )
    else:
        for item in cpu_results:
            ...
            add_result(
                f"{check_name}.CPU.{node}",
                status,
                f"Avg 5m CPU Usage: {usage:.2f}%",
                {"usage_percent": usage},
                rec,
            )
    ...
    if mem_results is None:
        add_result(
            f"{check_name}.Memory",
            "FAIL",
            "Failed to query Prometheus for Memory usage.",
        )
    else:
        for item in mem_results:
            ...
            add_result(
                f"{check_name}.Memory.{node}",
                status,
                f"Current Memory Usage: {usage:.2f}%",
                {"usage_percent": usage},
                rec,
            )
    ...
    if disk_results is None:
        add_result(
            f"{check_name}.Disk", "FAIL", "Failed to query Prometheus for Disk usage."
        )
    else:
        for item in disk_results:
            ...
            add_result(
                f"{check_name}.Disk.{node}",
                status,
                f"Root Disk Usage: {usage:.2f}%",
                {"usage_percent": usage},
                rec,
            )
...
def check_service_functionality():
    ...
    if not urls_to_check:
        add_result(
            ...
            recommendation="Set SERVICE_URLS_TO_CHECK environment variable.",
        )
        return
    ...
    for url in urls_to_check:
        ...
        try:
            ...
            )  # verify=False for self-signed certs, use carefully
            ...
            add_result(
                f"{check_name}.{service_name}",
                "FAIL",
                f"Request to {url} timed out.",
                {"url": url},
                f"Service at {url} is unresponsive or network issue exists.",
            )
            ...
            add_result(
                ...
                {"url": url},
                f"Service at {url} is down or unreachable.",
            )
        except Exception as e:
            add_result(
                f"{check_name}.{service_name}",
                ...
                {"url": url},
                f"Unexpected error checking service {url}.",
            )
...
def check_database_health():
    ...
    if not CHECK_DB:
        add_result(
            check_name,
            "SKIP",
            "Database check not enabled.",
            recommendation="Set CHECK_DB=true to enable.",
        )
        return
    if not all([PG_HOST, PG_USER, PG_PASSWORD, PG_DATABASE]):
        add_result(
            check_name,
            "FAIL",
            "PostgreSQL connection details incomplete.",
            recommendation="Set PG_HOST, PG_USER, PG_PASSWORD, PG_DATABASE.",
        )
        return
    ...
    try:
        ...
        add_result(
            check_name,
            "FAIL",
            "psycopg2 library not found.",
            recommendation="Install psycopg2 (`pip install psycopg2-binary`).",
        )
        return
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if result and result[0] == 1:
                add_result(
                    f"{check_name}.Connection",
                    "PASS",
                    f"Successfully connected to {PG_HOST} and executed simple query.",
                )
            else:
                add_result(
                    f"{check_name}.Connection",
                    "FAIL",
                    f"Connected to {PG_HOST} but simple query failed.",
                    recommendation="Check database status and permissions.",
                )
        ...
        with conn.cursor(cursor_factory=DictCursor) as cur:
            cur.execute(
                "SELECT current_setting('max_connections') AS max_conn, count(*) AS active_conn FROM pg_stat_activity;"
            )
            ...
            status, rec = "PASS", ""
            if (
                conn_pct >= PG_MAX_CONNECTIONS_WARN_PCT
            ):  # Use only WARN for connections, FAIL is too disruptive
                status = "WARN"
                rec = f"High connection count ({active_conn}/{max_conn}). Consider increasing max_connections or optimizing connection pooling."
            add_result(
                f"{check_name}.Connections",
                status,
                f"Active connections: {active_conn}/{max_conn} ({conn_pct:.1f}%)",
                {"active": active_conn, "max": max_conn, "percent": conn_pct},
                rec,
            )
            ...
            with conn.cursor(cursor_factory=DictCursor) as cur:
                ...
                if slow_queries:
                    details = [
                        {
                            "query": q["query"][:100] + "...",
                            "avg_ms": q["mean_exec_time"],
                        }
                        for q in slow_queries
                    ]
                    add_result(
                        f"{check_name}.SlowQueries",
                        "WARN",
                        f"Found {len(slow_queries)} slow queries (avg > {PG_SLOW_QUERY_THRESHOLD_MS}ms).",
                        details,
                        "Analyze query plans using EXPLAIN ANALYZE and consider indexing.",
                    )
                else:
                    add_result(
                        f"{check_name}.SlowQueries",
                        "PASS",
                        f"No slow queries found (avg > {PG_SLOW_QUERY_THRESHOLD_MS}ms).",
                    )
        except psycopg2.errors.UndefinedTable:
            add_result(
                f"{check_name}.SlowQueries",
                "INFO",
                "pg_stat_statements extension not enabled or accessible.",
                recommendation="Enable pg_stat_statements for slow query analysis.",
            )
        except Exception as e:
            add_result(
                f"{check_name}.SlowQueries", "WARN", f"Error checking slow queries: {e}"
            )
        ...
        add_result(
            ...
            recommendation="Check DB server status, network, credentials.",
        )
    except Exception as e:
...
def check_network_connectivity():
    ...
    if not targets_to_ping:
        add_result(
            check_name,
            "SKIP",
            "No network targets configured.",
            recommendation="Set NETWORK_TARGETS_TO_PING environment variable.",
        )
        return
    ...
    def parse_ping_output(output):
        ...
        rtt_match = re.search(
            r"rtt\s+min/avg/max/mdev\s*=\s*(\d+\.\d+)/(\d+\.\d+)/(\d+\.\d+)/(\d+\.\d+)\s*ms",
            output,
            re.IGNORECASE,
        )
        if not rtt_match:
            rtt_match = re.search(
                r"round-trip\s+min/avg/max\s*=\s*(\d+\.\d+)/(\d+\.\d+)/(\d+\.\d+)\s*ms",
                output,
                re.IGNORECASE,
            )
        packet_loss_str = loss_match.group(1) if loss_match else None
        packet_loss_pct = (
            float(packet_loss_str.replace("%", "")) if packet_loss_str else None
        )
        ...
        try:
            ...
            if packet_loss is None or avg_latency is None:
                add_result(
                    f"{check_name}.{target}",
                    "WARN",
                    f"Could not parse ping results for {target}.",
                    {"output": stdout},
                    "Check ping command output format.",
                )
                continue
            ...
            if packet_loss >= PACKET_LOSS_FAIL_PCT:
                ...
                status = "FAIL"
                rec = (
                    f"High latency to {target}. Check network load, switches, routing."
                )
            ...
            add_result(
                f"{check_name}.{target}",
                "FAIL",
                f"Ping to {target} timed out.",
                {"target": target},
                "Target may be down or network blocked.",
            )
        except subprocess.CalledProcessError:
            add_result(
                f"{check_name}.{target}",
                "FAIL",
                f"Ping command failed for {target} (e.g., unknown host).",
                {"target": target},
                "Check DNS, routing, firewall.",
            )
        except Exception as e:
            add_result(
                f"{check_name}.{target}",
...
def check_security_posture():
    ...
    if not CHECK_SECURITY:
        add_result(
            check_name,
            "SKIP",
            "Security checks not enabled.",
            recommendation="Set CHECK_SECURITY=true to enable.",
        )
        return
    ...
    if TRIVY_PATH and images_to_scan:
        for image in images_to_scan:
            try:
                ...
                add_result(
                    f"{check_name}.Trivy.{image.replace(':','_').replace('/','_')}",
                    "PASS",
                    f"No {TRIVY_SEVERITY} vulnerabilities found in {image}.",
                )
            except subprocess.CalledProcessError as e:
                ...
                if e.returncode == 1:
                    add_result(
                        f"{check_name}.Trivy.{image.replace(':','_').replace('/','_')}",
                        "FAIL",
                        f"Found {TRIVY_SEVERITY} vulnerabilities in {image}.",
                        {"image": image},
                        "Update image or dependencies. Run Trivy manually for details.",
                    )
                else:
                    add_result(
                        f"{check_name}.Trivy.{image.replace(':','_').replace('/','_')}",
                        "FAIL",
                        f"Trivy scan failed for {image} with exit code {e.returncode}.",
                        {"image": image},
                        "Check Trivy logs or run manually.",
                    )
            except Exception as e:
                add_result(
                    f"{check_name}.Trivy.{image.replace(':','_').replace('/','_')}",
                    "FAIL",
                    f"Error running Trivy scan for {image}: {e}",
                    {"image": image},
                )
    elif images_to_scan:
        add_result(
            f"{check_name}.Trivy",
            "SKIP",
            "Trivy path not found or not configured.",
            recommendation="Install Trivy and set TRIVY_PATH.",
        )

    # 2. Kube-bench Scan (Placeholder)
    if KUBE_BENCH_PATH:
        add_result(
            f"{check_name}.KubeBench",
            "INFO",
            "Kube-bench check placeholder.",
            recommendation="Implement kube-bench execution and result parsing.",
        )
        # try:
        ...
        add_result(
            f"{check_name}.KubeBench",
            "SKIP",
            "kube-bench path not found.",
            recommendation="Install kube-bench and set KUBE_BENCH_PATH.",
        )
    ...
    try:
        ...
        if os.path.exists(apt_log_path):
            ...
            add_result(
                f"{check_name}.OSUpdateAge",
                status,
                f"Last OS update detected {age_days} days ago.",
                {"last_update": last_update_dt.isoformat(), "age_days": age_days},
                rec,
            )
        else:
            add_result(
                f"{check_name}.OSUpdateAge",
                "INFO",
                "Cannot determine last OS update time (apt log not found).",
            )
    except Exception as e:
        add_result(
            f"{check_name}.OSUpdateAge", "WARN", f"Error checking OS update age: {e}"
        )
...
def check_backup_integrity():
    ...
    if not CHECK_BACKUPS:
        add_result(
            check_name,
            "SKIP",
            "Backup checks not enabled.",
            recommendation="Set CHECK_BACKUPS=true to enable.",
        )
        return
    ...
    if not local_dirs_to_check:
        add_result(
            f"{check_name}.Local",
            "SKIP",
            "No local backup directories configured.",
            recommendation="Set LOCAL_BACKUP_DIRS.",
        )
    else:
        ...
        for backup_dir in local_dirs_to_check:
            if not os.path.isdir(backup_dir):
                add_result(
                    f"{check_name}.Local.{os.path.basename(backup_dir)}",
                    "FAIL",
                    f"Backup directory not found: {backup_dir}",
                    recommendation="Verify backup directory path and volume mounts.",
                )
                continue
            ...
            try:
                ...
                if not backup_files:
                    add_result(
                        f"{check_name}.Local.{os.path.basename(backup_dir)}",
                        "FAIL",
                        f"No backup files found matching pattern '{BACKUP_FILE_PATTERN}' in {backup_dir}",
                        recommendation="Check backup job execution and file naming.",
                    )
                    continue
                ...
                if latest_file:
                    ...
                    add_result(
                        f"{check_name}.Local.{os.path.basename(backup_dir)}",
                        status,
                        f"Latest backup: {os.path.basename(latest_file)} (Age: {age_hours:.1f} hours)",
                        {"latest_file": latest_file, "age_hours": age_hours},
                        rec,
                    )
                ...
                add_result(
                    f"{check_name}.Local.{os.path.basename(backup_dir)}",
                    "FAIL",
                    f"Error checking backups in {backup_dir}: {e}",
                )
    ...
    if CHECK_S3_BACKUPS:
        if not all([S3_ENDPOINT_URL, S3_ACCESS_KEY, S3_SECRET_KEY, S3_BUCKET]):
            add_result(
                f"{check_name}.S3",
                "FAIL",
                "S3 configuration incomplete for backup check.",
                recommendation="Set S3_ENDPOINT_URL, S3_ACCESS_KEY, S3_SECRET_KEY, S3_BUCKET.",
            )
        else:
            try:
                import boto3
                ...
                latest_s3_obj = None
                latest_s3_mtime = datetime.datetime.fromtimestamp(
                    0, datetime.timezone.utc
                )  # Timezone aware epoch
                ...
                if latest_s3_obj:
                    ...
                    status, rec = "PASS", ""
                    if latest_s3_mtime < (
                        now_utc - datetime.timedelta(hours=MAX_BACKUP_AGE_HOURS)
                    ):  # Compare UTC times
                        status = "FAIL"
                    ...
                    add_result(
                        f"{check_name}.S3",
                        status,
                        f"Latest S3 backup object: {latest_s3_obj} (Age: {age_hours_s3:.1f} hours)",
                        {"latest_object": latest_s3_obj, "age_hours": age_hours_s3},
                        rec,
                    )
                else:
                    add_result(
                        f"{check_name}.S3",
                        "FAIL",
                        f"No backup objects found in s3://{S3_BUCKET}/{S3_PREFIX}",
                        recommendation="Check offsite backup job execution.",
                    )
                ...
                add_result(
                    ...
                    recommendation="Install boto3 (`pip install boto3`).",
                )
            except ClientError as e:
                add_result(
                    f"{check_name}.S3",
                    "FAIL",
                    f"S3 ClientError checking backups: {e}",
                    recommendation="Check S3 credentials, endpoint, bucket name, and permissions.",
                )
            except Exception as e:
                add_result(
                    f"{check_name}.S3", "FAIL", f"Error checking S3 backups: {e}"
                )

    # 3. Backup Recoverability (Placeholder)
    add_result(
        f"{check_name}.Recoverability",
        "INFO",
        "Backup recoverability check placeholder.",
        recommendation="Implement periodic test restores (manual or scripted) to verify backup integrity.",
    )
...
def generate_report():
    ...
    for result in health_results:
        ...
        if status == "PASS":
            ...
            if final_status == "PASS":
                final_status = "WARN"  # Downgrade overall status
        elif status == "FAIL":
    ...
    full_report = {"summary": summary, "results": health_results}

    # Print Summary to Console
    logger.info("--- Health Check Summary ---")
    logger.info(f"Overall Status: {summary['overall_status']}")
    logger.info(
        f"Checks: Total={summary['total_checks']}, Pass={summary['pass_count']}, Warn={summary['warn_count']}, Fail={summary['fail_count']}, Skip={summary['skip_count']}"
    )
    if summary["fail_count"] > 0:
        ...
        for r in health_results:
            if r["status"] == "FAIL":
                logger.error(
                    f"  - {r['check']}: {r['message']} {r.get('recommendation','')}"
                )
