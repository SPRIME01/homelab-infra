...
ALL_NODES = list(set([n for n in CONTROL_PLANE_NODES + WORKER_NODES if n])) # Unique list
...
ANSIBLE_PLAYBOOK_OS_UPDATE = os.getenv("ANSIBLE_PLAYBOOK_OS_UPDATE", "/path/to/ansible/update_os.yml")
# Or, if using SSH directly (less recommended)
SSH_USER = os.getenv("SSH_USER", "your_ssh_user")
SSH_OPTIONS = os.getenv("SSH_OPTIONS", "-o StrictHostKeyChecking=no") # Example, adjust as needed

# Kubernetes Updates
DO_K8S_UPDATE = os.getenv("DO_K8S_UPDATE", "false").lower() == "true"
TARGET_K8S_VERSION = os.getenv("TARGET_K8S_VERSION") # e.g., "v1.28.5" (must include 'v')
...
APP_IMAGE_UPDATES = dict(item.split('=') for item in os.getenv("APP_IMAGE_UPDATES", "").split(',') if '=' in item)

# Safety & Coordination
AUTO_APPROVE = os.getenv("AUTO_APPROVE", "false").lower() == "true" # Auto-approve prompts
PERFORM_DRAIN = os.getenv("PERFORM_DRAIN", "true").lower() == "true" # Drain nodes during updates
MAX_NODE_UPDATE_FAILURES = int(os.getenv("MAX_NODE_UPDATE_FAILURES", "1")) # Max nodes allowed to fail update before aborting
ROLLBACK_ON_FAILURE = os.getenv("ROLLBACK_ON_FAILURE", "false").lower() == "true" # Attempt app rollback on failure

# --- Logging Setup ---
if not os.path.exists(LOG_DIR):
    os.makedirs(LOG_DIR)
log_filename = os.path.join(LOG_DIR, f"system_update_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
logging.basicConfig(
    level=logging.INFO,
...
logger.critical("Running this automatically is RISKY and can cause significant downtime or data loss if not carefully configured and tested.")
...
def run_command(command, check=True, timeout=None, capture_output=True, shell=False, log_output=True, env=None):
    ...
    try:
        ...
        if log_output:
            if stdout_log: logger.info(f"Command stdout:\n{stdout_log}")
            # Log stderr as warning even on success, as commands might output info there
            if stderr_log: logger.warning(f"Command stderr:\n{stderr_log}")
        else:
        ...
        if stdout_log: logger.error(f"Failed command stdout:\n{stdout_log}")
        if stderr_log: logger.error(f"Failed command stderr:\n{stderr_log}")
        raise
...
def run_ansible_playbook(playbook_path, inventory_path, extra_vars=None):
    ...
    if not os.path.exists(playbook_path) or not os.path.exists(inventory_path):
        logger.error(f"Ansible playbook or inventory not found: {playbook_path}, {inventory_path}")
...
def check_node_status(expected_count):
    ...
    try:
        ...
        if ready_nodes >= expected_count:
            ...
            logger.error(f"Expected {expected_count} Ready nodes, but found {ready_nodes}.")
            return False
    except Exception as e:
...
def check_deployment_status(namespace="--all-namespaces", min_ready_percent=100):
    ...
    try:
        ...
        stdout, _ = run_command([
            "kubectl", "get", "deployments"] + ns_arg + ["--context", KUBECTL_CONTEXT, "-o", "json"]
        , timeout=60)
        ...
        for deploy in deployments:
            ...
            available_replicas = deploy.get("status", {}).get("availableReplicas", 0) # Consider available too
            ...
            if spec_replicas == 0: # Scaled down intentionally
                 ...
                 if ready_replicas >= spec_replicas and ready_percent >= min_ready_percent:
                      is_healthy = True

            if is_healthy:
                 logger.info(f"Deployment {ns}/{name}: Healthy ({ready_replicas}/{spec_replicas} ready)")
            else:
                 logger.warning(f"Deployment {ns}/{name}: UNHEALTHY ({ready_replicas}/{spec_replicas} ready, {available_replicas} available)")
                 all_healthy = False
...
def drain_node(node):
    ...
    run_command([
        "kubectl", "drain", node, "--context", KUBECTL_CONTEXT,
        "--ignore-daemonsets", "--delete-emptydir-data", "--force",
        "--timeout=5m" # Add a timeout
    ], timeout=360) # Timeout for the command itself
...
def update_os_packages():
    ...
    if not ALL_NODES:
        logger.warning("No nodes defined (CONTROL_PLANE_NODES, WORKER_NODES). Skipping OS updates.")
        return True
    ...
    try:
        if os.path.exists(ANSIBLE_PLAYBOOK_OS_UPDATE):
            ...
            logger.warning("Ansible playbook not found. Attempting OS updates via SSH (less recommended).")
            logger.warning("This basic SSH method does NOT handle reboots gracefully. Use Ansible for that.")
            update_cmd = "sudo apt update && sudo apt upgrade -y" # Debian/Ubuntu example
        ...
        logger.warning("Reboot coordination is NOT implemented in the basic SSH method. Use Ansible or add logic.")
        return True
    except Exception as e:
...
def update_k8s_control_plane():
    ...
    if not TARGET_K8S_VERSION or not TARGET_K8S_VERSION.startswith('v'):
        logger.error(f"TARGET_K8S_VERSION ('{TARGET_K8S_VERSION}') is not set or invalid. Must start with 'v'.")
        return False
    ...
    try:
        ...
        run_ssh_command(first_cp_node, f"sudo kubeadm upgrade plan {TARGET_K8S_VERSION}")
        if not AUTO_APPROVE:
            input(f"Review the upgrade plan for {TARGET_K8S_VERSION}. Press Enter to apply or Ctrl+C to abort...")
        ...
        run_ssh_command(first_cp_node, f"sudo kubeadm upgrade apply {TARGET_K8S_VERSION} --yes")
        ...
        for node in other_cp_nodes:
            logger.info(f"Updating kubeadm and kubelet on other control plane node: {node}")
            ...
            run_ssh_command(node, "sudo systemctl daemon-reload && sudo systemctl restart kubelet")
        ...
        run_ssh_command(first_cp_node, "sudo systemctl daemon-reload && sudo systemctl restart kubelet")
        ...
        logger.error("Manual intervention likely required. Check kubeadm logs and component statuses.")
        return False
...
def update_k8s_workers():
    ...
    if not WORKER_NODES:
        ...
        return True
    if not TARGET_K8S_VERSION or not TARGET_K8S_VERSION.startswith('v'):
        logger.error(f"TARGET_K8S_VERSION ('{TARGET_K8S_VERSION}') is not set or invalid.")
        return False
    ...
    for node in WORKER_NODES:
        ...
        try:
            ...
            run_ssh_command(node, update_worker_tools_cmd.replace(f"kubelet={TARGET_K8S_VERSION[1:]}-00","")) # Only kubeadm first
            ...
            run_ssh_command(node, update_worker_tools_cmd.replace(f"kubeadm={TARGET_K8S_VERSION[1:]}-00","")) # Only kubelet now
            ...
            if not check_node_status(len(ALL_NODES)): # Check overall cluster health
                 logger.warning(f"Node {node} updated, but cluster health check failed post-uncordon.")
            ...
            try:
                 ...
                 logger.error(f"Failed to uncordon node {node} after update failure: {uncordon_e}")

            if failed_nodes >= MAX_NODE_UPDATE_FAILURES:
                logger.critical(f"Reached maximum allowed node update failures ({MAX_NODE_UPDATE_FAILURES}). Aborting worker updates.")
                return False
            else:
...
def update_container_images():
    ...
    logger.warning("Consider GitOps tools (Argo CD Image Updater, Flux, Renovate) for robust image updates.")
    ...
    for resource_id, target_image in APP_IMAGE_UPDATES.items():
        try:
            ...
            namespace = "default"
            if ':' in name:
                 namespace, name = name.split(':') # Allow format like deployment:ns/name

            logger.info(f"Updating {kind} '{namespace}/{name}' to image '{target_image}'...")
            ...
            container_name = name
            run_command([
                "kubectl", "set", "image", kind, name, f"{container_name}={target_image}",
                "-n", namespace, "--context", KUBECTL_CONTEXT, "--record" # Record helps with rollback
            ])
            logger.info(f"Triggered update for {kind} '{namespace}/{name}'. Monitoring rollout...")
            # Wait and check rollout status
            run_command([
                 "kubectl", "rollout", "status", kind, name, "-n", namespace,
                 "--context", KUBECTL_CONTEXT, "--timeout=5m"
            ], timeout=310)
            logger.info(f"Rollout finished for {kind} '{namespace}/{name}'.")
            # Add post-update validation specific to the app if possible

        except Exception as e:
            logger.error(f"Failed to update {kind} '{namespace}/{name}' to image '{target_image}': {e}")
            success = False
            rollbacks_needed.append({"kind": kind, "name": name, "namespace": namespace})

    if not success and ROLLBACK_ON_FAILURE:
        logger.warning("One or more application updates failed. Attempting rollbacks...")
...
def rollback_application(kind, namespace, name):
    ...
    try:
        ...
        logger.info(f"Rollback command executed for {kind} '{namespace}/{name}'. Monitor status manually.")
    ...
    try:
        ...
        if DO_K8S_UPDATE:
            ...
            if update_k8s_control_plane():
                ...
                logger.info("Waiting after control plane update before updating workers...")
                time.sleep(60)
                if not check_node_status(len(ALL_NODES)) or not check_deployment_status(namespace="kube-system"):
                     raise RuntimeError("Cluster health check failed after control plane update.")
                ...
                if update_k8s_workers():
                    ...
                    overall_success = False
                    logger.critical("Kubernetes worker update failed. Aborting further updates.")
                    raise RuntimeError("Kubernetes Worker Update Failed.")
            else:
                overall_success = False
                logger.critical("Kubernetes control plane update failed. Aborting further updates.")
        ...
        if ROLLBACK_ON_FAILURE and "AppUpdate" in steps_executed and "AppUpdate" not in steps_succeeded:
             logger.warning("Attempting application rollbacks due to script error during/after app updates.")
             ...
             for resource_id, _ in APP_IMAGE_UPDATES.items():
                  try:
                       kind, name = resource_id.split('/')
                       namespace = "default"
                       if ':' in name: namespace, name = name.split(':')
...
if __name__ == "__main__":
    ...
    tools = ["kubectl", "ssh"]
    if DO_OS_UPDATE and os.path.exists(ANSIBLE_PLAYBOOK_OS_UPDATE): tools.append("ansible-playbook")
    if DO_K8S_UPDATE: tools.append("kubeadm") # Assumes kubeadm is run via SSH

    missing = [tool for tool in tools if shutil.which(tool) is None]
    if missing:
        logger.critical(f"Missing required command-line tools: {', '.join(missing)}. Please install them.")
        sys.exit(2)
