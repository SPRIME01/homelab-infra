...
# Thresholds
MIN_READY_PERCENT = int(os.getenv("MIN_READY_PERCENT", "80")) # Minimum % of replicas that must be ready
MAX_RESTARTS_THRESHOLD = int(os.getenv("MAX_RESTARTS_THRESHOLD", "5")) # Pod restart count threshold to trigger alert/action
...
def check_service_health(kind, namespace, name):
    ...
    try:
        ...
        spec_replicas = json.loads(output).get("spec", {}).get("replicas", 1) # Default to 1 if not specified
        ready_replicas = status.get("readyReplicas", 0) or status.get("currentReplicas", 0) # STS uses currentReplicas sometimes
        available_replicas = status.get("availableReplicas", 0)

        logging.info(f"{kind} {namespace}/{name}: Spec={spec_replicas}, Ready={ready_replicas}, Available={available_replicas}")
        ...
        if spec_replicas > 0:
            ...
            if ready_percent < MIN_READY_PERCENT:
                ...
                # Attempt restart
                try:
                    logging.warning(f"Attempting rollout restart for {kind} '{namespace}/{name}' due to low readiness.")
                    run_command([
                        "kubectl", "rollout", "restart", kind, name, "-n", namespace,
                        "--context", KUBECTL_CONTEXT
                    ])
                except Exception as e:
                    logging.error(f"Failed to trigger rollout restart for {kind} '{namespace}/{name}': {e}")
                    send_alert(f"Failed to trigger rollout restart for {kind} '{namespace}/{name}'.")

        # Check for high pod restarts (more complex, requires getting pods)
        pod_output = run_command([
            "kubectl", "get", "pods", "-n", namespace,
            "--context", KUBECTL_CONTEXT,
            "-l", json.loads(output)["spec"]["selector"]["matchLabels"], # Use label selector
            "-o", "json"
        ], check=False) # Don't fail if no pods found immediately after restart
