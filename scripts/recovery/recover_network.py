...
NODE_TARGETS = os.getenv("NODE_TARGETS", "").split(
    ","
...
def check_network_connectivity():
            ...
            try:
                ...
                run_command(
                    [
                        ...
                        "-c",
                        PING_COUNT,
                        f"-W{int(PING_TIMEOUT)//int(PING_COUNT)}",
                        target,
                    ],
    ...
    logging.info(
        f"Checking DNS resolution and external connectivity to {EXTERNAL_TARGET_HOST}..."
    )
    try:
    ...
    if not network_ok:
        logging.warning(
            "Network issues detected. Automated recovery is generally unsafe."
        )
        logging.warning(
            "Consider checking: Node networking services, DNS configuration (CoreDNS logs), Firewall rules, Physical connections."
        )
