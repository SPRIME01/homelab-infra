...
NODE_CHECK_TIMEOUT = os.getenv(
    "NODE_CHECK_TIMEOUT", "10s"
)  # Timeout for kubectl get node status
ALERT_COMMAND = os.getenv(
    "ALERT_COMMAND"
)  # Optional command to run for alerting (e.g., webhook)
...
def check_node_status():
    ...
    try:
        ...
        output = run_command(
            ...
            timeout=int(NODE_CHECK_TIMEOUT.replace("s", "")),
        )  # Convert timeout string
        ...
        for node_line in nodes:
            ...
            if len(parts) != 2:
                logging.warning(
                    f"Unexpected format for node status line: '{node_line}'"
                )
                continue
            node_name, ready_status = parts
            if ready_status != "True":
                logging.warning(
                    f"Node '{node_name}' is not Ready (Status: {ready_status})."
                )
        ...
        if unhealthy_nodes:
            message = (
                f"Unhealthy Kubernetes nodes detected: {', '.join(unhealthy_nodes)}"
            )
            send_alert(message)
            ...
            logging.warning(
                "Further recovery actions (reboot, drain) typically require manual intervention or dedicated infrastructure automation."
            )
        ...
        send_alert(
            "Timeout checking Kubernetes node status. Control plane might be unresponsive."
        )
        return False
    except Exception as e:
