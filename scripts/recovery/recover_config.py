...
def get_last_applied_config_time(kind, namespace, name):
    ...
    try:
        ...
        last_applied_str = annotations.get(
        ...
        logging.warning(
            f"Could not determine last update time for {kind} {namespace}/{name}: {e}"
        )
        return None
...
def check_recent_configs():
    ...
    logging.info(
        f"Checking for unhealthy resources updated within the last {CHECK_WINDOW_MINUTES} minutes."
    )
        ...
        try:
            ...
            for item in items:
                ...
                if last_update_time and last_update_time > check_cutoff_time:
                    logging.info(
                        f"{kind} '{namespace}/{name}' was updated recently ({last_update_time}). Checking health..."
                    )
                    ...
                    if spec_replicas > 0 and ready_replicas < spec_replicas:
                        ...
                        if AUTO_ROLLBACK:
                            logging.warning(
                                f"Attempting automatic rollback for {kind} '{namespace}/{name}' due to unhealthy state after recent update."
                            )
                            try:
                                run_command(
                                    [
                                ...
                                send_alert(
                                    f"Automatic rollback initiated for {kind} '{namespace}/{name}'."
                                )
                            except Exception as e:
                                logging.error(
                                    f"Automatic rollback failed for {kind} '{namespace}/{name}': {e}"
                                )
                                send_alert(
                                    f"Automatic rollback FAILED for {kind} '{namespace}/{name}'."
                                )
                        else:
                            logging.warning(
                        ...
                        logging.info(
                            f"Recently updated {kind} '{namespace}/{name}' appears healthy."
                        )
...
if __name__ == "__main__":
    if not check_recent_configs():
        logging.warning(
            "Potential configuration issues detected in recently updated resources."
        )
        # Consider linking to Git history or backup system for manual restore
        logging.warning(
            "Check Git history in config backup repo or use 'kubectl rollout history' for manual investigation/rollback."
        )
        sys.exit(1)
    else:
        logging.info(
            "No obvious configuration issues found in recently updated resources."
        )
        sys.exit(0)
