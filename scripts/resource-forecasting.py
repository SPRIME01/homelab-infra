...
FORECAST_DISK_WARN_THRESHOLD = float(
    os.getenv("FORECAST_DISK_WARN_THRESHOLD", "70")
)  # % (Root FS)
...
def query_prometheus_range(query, duration, step):
    ...
    logger.info(
        f"Querying Prometheus Range API (duration={duration}, step={step}): {query}"
    )
    try:
        response = requests.get(
            api_endpoint, params=params, timeout=120
        )  # Longer timeout for range queries
        ...
        if result["status"] == "success":
            ...
            logger.error(
                f"Prometheus range query failed: {result.get('error', 'Unknown error')}"
            )
            return None
    ...
    for result in prom_results:
        ...
        for timestamp, value in values:
            try:
                ...
                logger.warning(
                    f"Skipping non-numeric value '{value}' for {metric_name} at timestamp {timestamp}"
                )
                continue

    if not data:
        logger.warning(
            f"No valid numeric data points found for {metric_name} after processing."
        )
        return pd.DataFrame()
...
def analyze_and_forecast(df, metric_name, forecast_days):
    ...
    if df.empty or metric_name not in df.columns:
        logger.warning(
            f"DataFrame is empty or missing '{metric_name}' column. Skipping forecast."
        )
        return forecasts
    ...
    if len(metric_series) < 10:  # Need sufficient data points for regression
        logger.warning(
            f"Insufficient data points ({len(metric_series)}) for {metric_name} trend analysis. Skipping forecast."
        )
        return forecasts
    ...
    try:
        ...
        results = model.fit()
        logger.info(
            f"Linear Regression Summary for {metric_name}:\n{results.summary()}"
        )

        # Forecast future values
        last_timestamp_sec = (
            metric_series.index.max() - metric_series.index.min()
        ).total_seconds()
        forecast_end_sec = last_timestamp_sec + forecast_days * 86400
        # Create future time points (e.g., daily for forecast horizon)
        forecast_X_sec = np.linspace(
            last_timestamp_sec, forecast_end_sec, forecast_days + 1
        )
        ...
        forecasts["trend_slope"] = results.params[
            1
        ]  # Slope indicates increase/decrease per second
        forecasts["current_value"] = y[-1]  # Last observed value
        forecasts["forecast_value_end"] = forecast_values[
            -1
        ]  # Forecasted value at the end of the horizon
        ...
        logger.info(
            f"Forecast for {metric_name}: Current={y[-1]:.2f}, TrendSlope={slope_per_day:.4f}/day, ForecastEnd={forecast_values[-1]:.2f} in {forecast_days} days"
        )
...
def generate_recommendations(forecasts):
    ...
    if "CPU" in forecasts:
        ...
        if fc.get("forecast_value_end", -1) > FORECAST_CPU_WARN_THRESHOLD:
            rec = (
                f"CPU usage forecast ({fc['forecast_value_end']:.1f}%) exceeds threshold ({FORECAST_CPU_WARN_THRESHOLD}%) "
            ...
            recommendations.append(
                {"resource": "CPU", "severity": "WARN", "message": rec}
            )
    ...
    if "Memory" in forecasts:
        ...
        if fc.get("forecast_value_end", -1) > FORECAST_MEM_WARN_THRESHOLD:
            rec = (
                f"Memory usage forecast ({fc['forecast_value_end']:.1f}%) exceeds threshold ({FORECAST_MEM_WARN_THRESHOLD}%) "
                f"in {fc['forecast_period_days']} days. Trend slope: {fc.get('trend_slope', 0)*86400:.2f}%/day. "
                f"Recommendation: Investigate memory usage patterns (leaks?) or consider adding RAM/adjusting limits."
            )
            recommendations.append(
                {"resource": "Memory", "severity": "WARN", "message": rec}
            )
    ...
    if "Disk" in forecasts:
        ...
        if fc.get("forecast_value_end", -1) > FORECAST_DISK_WARN_THRESHOLD:
            rec = (
                f"Disk usage forecast ({fc['forecast_value_end']:.1f}%) exceeds threshold ({FORECAST_DISK_WARN_THRESHOLD}%) "
                f"in {fc['forecast_period_days']} days. Trend slope: {fc.get('trend_slope', 0)*86400:.2f}%/day. "
                f"Recommendation: Plan disk space cleanup (logs, images) or expand storage capacity."
            )
            recommendations.append(
                {"resource": "Disk", "severity": "WARN", "message": rec}
            )
...
def generate_report(all_forecasts, recommendations):
    ...
    for resource, fc in all_forecasts.items():
        ...
        logger.info(
            f"  Forecast ({fc.get('forecast_period_days', 'N/A')} days): {fc.get('forecast_value_end', 'N/A'):.2f}"
        )
    ...
    if recommendations:
        ...
        logger.info(
            "No immediate recommendations based on current forecasts and thresholds."
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.critical(
            f"Missing required Python library: {e.name}. Please install it."
        )
