...
# --- Configuration ---
KUBECTL_CONTEXT = os.getenv("KUBECTL_CONTEXT", "homelab-cluster") # Context for test cluster if different
RECOVERY_SCRIPT_DIR = os.getenv("RECOVERY_SCRIPT_DIR", "../scripts/recovery") # Path to recovery scripts
REPORT_DIR = os.getenv("REPORT_DIR", "./recovery_test_reports")
DEFAULT_TIMEOUT = int(os.getenv("DEFAULT_TIMEOUT", "300")) # Default timeout for steps in seconds
...
logger.critical("Running this against a PRODUCTION environment is EXTREMELY RISKY and can cause DATA LOSS or OUTAGES.")
logger.critical("ONLY run this in a DEDICATED, ISOLATED test environment or during scheduled maintenance with full backups.")
...
class StopDeploymentScenario(FailureScenario):
    ...
    def simulate(self):
        self.logger.warning(f"Simulating failure: Scaling deployment '{self.namespace}/{self.deployment_name}' to 0 replicas.")
        try:
            # Get original replica count
            stdout, _ = run_command([
                "kubectl", "get", "deployment", self.deployment_name, "-n", self.namespace,
                "--context", KUBECTL_CONTEXT, "-o", "jsonpath={.spec.replicas}"
            ])
            ...
            # Scale down
            run_command([
                "kubectl", "scale", "deployment", self.deployment_name, "--replicas=0",
                "-n", self.namespace, "--context", KUBECTL_CONTEXT
            ])
    ...
    def cleanup(self):
        if self.original_replicas is not None:
            self.logger.info(f"Cleaning up simulation: Scaling deployment '{self.namespace}/{self.deployment_name}' back to {self.original_replicas} replicas.")
            try:
                run_command([
                    "kubectl", "scale", "deployment", self.deployment_name, f"--replicas={self.original_replicas}",
                    "-n", self.namespace, "--context", KUBECTL_CONTEXT
                ])
            except Exception as e:
...
class ExecuteScriptRecovery(RecoveryProcedure):
    ...
    def execute(self):
        self.logger.info(f"Executing recovery script: {self.script_path} with args {self.script_args}")
...
class DeploymentReadyValidation(ValidationStep):
    ...
    def __init__(self, name, description, namespace, deployment_name, expected_replicas=None, min_ready_percent=100):
        super().__init__(name, description)
        self.namespace = namespace
        self.deployment_name = deployment_name
        self.expected_replicas = expected_replicas # Optional: check against specific number
        self.min_ready_percent = min_ready_percent

    def validate(self):
        self.logger.info(f"Validating deployment '{self.namespace}/{self.deployment_name}' readiness...")
        try:
            stdout, _ = run_command([
                "kubectl", "get", "deployment", self.deployment_name, "-n", self.namespace,
            ...
            target_replicas = self.expected_replicas if self.expected_replicas is not None else spec_replicas

            if target_replicas == 0: # If expecting 0 replicas (e.g., testing scale down)
                 is_valid = (ready_replicas == 0)
            elif spec_replicas == 0: # If spec is 0 but we expect > 0 (should not happen in recovery)
                 is_valid = False
            else:
                 ready_percent = (ready_replicas / spec_replicas) * 100
                 is_valid = (ready_replicas >= target_replicas and ready_percent >= self.min_ready_percent)

            if is_valid:
                self.logger.info(f"Validation successful: {ready_replicas}/{spec_replicas} replicas ready.")
                return True
            else:
                self.logger.error(f"Validation failed: {ready_replicas}/{spec_replicas} replicas ready. Expected >= {target_replicas} and >= {self.min_ready_percent}%.")
                return False
        except Exception as e:
...
class RecoveryTester:
    ...
    def run_test(self, scenario: FailureScenario, procedure: RecoveryProcedure, validations: list[ValidationStep]):
        ...
        try:
            ...
            result["steps"].append({
                "step": "simulate", "name": scenario.name, "success": True,
                "duration": time.monotonic() - step_start, "error": None
            })
            ...
            result["steps"].append({
                "step": "recover", "name": procedure.name, "success": True,
                "duration": time.monotonic() - step_start, "error": None
            })
            ...
            for validation in validations:
                ...
                validation_step_results.append({
                    "step": "validate", "name": validation.name, "success": step_success,
                    "duration": time.monotonic() - step_start, "error": None if step_success else "Validation check failed"
                })
                if not step_success:
            ...
            logger.info(f"Validation step completed. Overall success: {validation_success}")

        except Exception as e:
            step_name = "unknown"
            current_step_result = {"success": False, "duration": time.monotonic() - step_start, "error": str(e)}
            if not simulate_success:
                ...
                recovery_end_time = datetime.now() # Record end time even on failure
                 ...
                 if not any(s["step"] == "validate" and s["error"] for s in result["steps"]):
                      result["steps"].append({
                           "step": "validate", "name": "framework_error", **current_step_result
                      })

            logger.error(f"Test failed during step '{step_name}': {e}")
            # Ensure failed step is recorded if not already
            if not any(s["step"] == step_name and not s["success"] for s in result["steps"]):
                 # Find the step dict if it exists, update it, otherwise add it
                 step_dict = next((s for s in result["steps"] if s["step"] == step_name), None)
                 if step_dict:
                      step_dict.update(current_step_result)
                 else:
                      # Need a name if adding fresh
                      name = scenario.name if step_name == "simulate" else procedure.name if step_name == "recover" else "validation_framework"
                      result["steps"].append({"step": step_name, "name": name, **current_step_result})
            ...
            try:
                scenario.cleanup()
                result["steps"].append({
                    "step": "cleanup", "name": scenario.name, "success": True,
                    "duration": time.monotonic() - step_start, "error": None
                })
                ...
                result["steps"].append({
                    "step": "cleanup", "name": scenario.name, "success": False,
                    "duration": time.monotonic() - step_start, "error": str(e)
                })

            end_time = datetime.now()
            result["end_time"] = end_time.isoformat()
            result["success"] = simulate_success and recovery_success and validation_success
            ...
            if recovery_start_time and recovery_end_time:
                 ...
                 result["recovery_duration_seconds"] = (recovery_end_time - recovery_start_time).total_seconds()

            self.results.append(result)
            logger.info(f"--- Test Finished: {test_name} | Success: {result['success']} | Duration: {result['total_duration_seconds']:.2f}s ---")
    ...
    def generate_summary_report(self):
        ...
        for i, result in enumerate(self.results):
            ...
            logger.info(f"  Total Duration: {duration:.2f}s" if isinstance(duration, float) else f"  Total Duration: {duration}")
            logger.info(f"  Recovery Duration: {recovery_duration:.2f}s" if isinstance(recovery_duration, float) else f"  Recovery Duration: {recovery_duration}")
            if not result["success"]:
                 failed_step = next((s for s in result["steps"] if not s["success"]), None)
                 if failed_step:
                      logger.warning(f"  Failed Step: {failed_step['step']} - {failed_step['name']}")
                      if failed_step['error']: logger.warning(f"    Error: {failed_step['error']}")
...
def define_test_cases():
    ...
    test_cases.append({"scenario": scenario1, "procedure": recovery1, "validations": [validation1]})
...
if __name__ == "__main__":
    ...
    for test_case in test_definitions:
        try:
             ...
             logger.critical(f"Unhandled exception during test execution for {test_case.get('scenario',{}).name}: {e}", exc_info=True)
             ...
             tester.results.append({
                  ...
                  "steps": [{"step": "framework", "name": "critical_error", "success": False, "error": str(e)}],
                  "success": False,
