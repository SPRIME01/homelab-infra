...
SERVICES = {
    "target_allowed": {
        "name": "target-svc-allowed",
        "namespace": NAMESPACES["allow_specific"],
        "port": 80,
    },
    "target_denied": {
        "name": "target-svc-denied",
        "namespace": NAMESPACES["allow_specific"],
        "port": 80,
    },
    "target_iso_a": {
        "name": "target-svc-iso-a",
        "namespace": NAMESPACES["isolated_a"],
        "port": 80,
    },
    "target_iso_b": {
        "name": "target-svc-iso-b",
        "namespace": NAMESPACES["isolated_b"],
        "port": 80,
    },
}
...
def create_deployment(name, namespace, labels=None):
    ...
    try:
        ...
        if e.status == 409:  # Already exists
            logging.info(
                f"Deployment '{name}' already exists in namespace '{namespace}'."
            )
        else:
            raise
...
def create_service(name, namespace, port, labels=None):
    ...
    service = client.V1Service(
        ...
        spec=client.V1ServiceSpec(
            selector=svc_labels,
            ports=[
                client.V1ServicePort(protocol="TCP", port=port, target_port=80)
            ],  # Assumes target port 80
        ),
    )
    try:
...
def can_connect_from_pod(
    pod_name, namespace, target_host, target_port, timeout=DEFAULT_TIMEOUT_SECONDS
):
...
def test_default_deny_effectiveness(tester_pod):
    ...
    assert not can_connect_from_pod(
        pod_name, namespace, target_host, target_port
...
def test_specific_allow_rules(tester_pod):
    ...
    assert can_connect_from_pod(
        pod_name, namespace, allowed_host, allowed_port
    ...
    assert not can_connect_from_pod(
        pod_name, namespace, denied_host, denied_port
...
def test_namespace_isolation_a_to_b(tester_pod):
    ...
    logging.info(
        f"Testing namespace isolation from {namespace} to {target_svc_info['namespace']}..."
    )
    ...
    logging.info(
        f"Namespace isolation test ({namespace} -> {target_svc_info['namespace']}) passed."
    )
...
def test_egress_control(tester_pod):
    ...
    assert can_connect_from_pod(
        pod_name, namespace, allowed_host, int(allowed_port)
    ...
    assert not can_connect_from_pod(
        pod_name, namespace, denied_host, int(denied_port)
