...
def test_internal_service_discovery_and_dns():
    for svc in INTERNAL_SERVICES:
        ...
        assert can_connect(
            svc["host"], svc["port"]
        ), f"Cannot connect to {svc['name']} at {svc['host']}:{svc['port']}"
...
def test_network_policy_enforcement():
    ...
    ), f"NetworkPolicy failed: unexpected access to {blocked_host}:{blocked_port}"


@pytest.mark.skipif(
    not SERVICE_MESH_ENDPOINTS, reason="Service mesh endpoints not configured"
)
...
def test_api_gateway_routing(url):
    try:
        ...
        assert resp.status_code in (
            ...
            401,
            403,
        ), f"API Gateway routing failed for {url} (status {resp.status_code})"
    except Exception as e:
